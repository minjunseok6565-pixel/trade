...
_INGEST_TURN_BACKFILLED = False


def normalize_player_ids(game_state: dict, *, allow_legacy_numeric: bool = True) -> dict:
    """
    Normalize identifiers so *player_id is always a string* and unique.
    ...
    Policy:
    - Keys of players are canonical player_id strings.
    ...
    for raw_key, raw_meta in players.items():
        ...
        if "player_id" not in meta:
            ...
            try:
                ...
                raise ValueError(
                    f"players[{raw_key!r}].player_id is invalid: {meta.get('player_id')!r}"
                )
            if meta_pid != pid:
                raise ValueError(
    ...
    if conflicts:
        ...
        raise ValueError(
            "Duplicate player_id keys detected while normalizing players.\n"
    ...
    if invalid:
        ...
        raise ValueError(
            "Invalid player keys detected while normalizing players.\n"
...
def _backfill_ingest_turns_once(game_state: dict) -> None:
    ...
    phase_results = game_state.get("phase_results", {})
    ...
    if isinstance(season_history, dict):
        for record in season_history.values():
            ...
            if isinstance(regular, dict):
                games = regular.get("games", [])
                if isinstance(games, list):
                    all_games.extend([g for g in games if isinstance(g, dict)])
            phases = record.get("phases")
            if isinstance(phases, dict):
                for container in phases.values():
                    if not isinstance(container, dict):
                        continue
                    games = container.get("games", [])
                    if isinstance(games, list):
...
def _ensure_ingest_turn_backfilled(game_state: dict) -> None:
    """Ensure ingest_turn backfill runs once per process."""
    global _INGEST_TURN_BACKFILLED
    if _INGEST_TURN_BACKFILLED:
        return
    _backfill_ingest_turns_once(game_state)
    _INGEST_TURN_BACKFILLED = True


def ensure_ingest_turn_backfilled_once_startup(game_state: dict) -> None:
    """Run ingest_turn backfill once per process (startup-only)."""
    _ensure_ingest_turn_backfilled(game_state)
