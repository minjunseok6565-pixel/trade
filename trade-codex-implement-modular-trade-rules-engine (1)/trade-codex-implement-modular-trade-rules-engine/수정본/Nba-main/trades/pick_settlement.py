...
def _get_db_path(game_state: dict) -> str:
    ...
    if not db_path:
        raise ValueError("game_state['league']['db_path'] is required to settle draft picks")
    return db_path
...
def settle_draft_year(
    ...
    with LeagueRepo(db_path) as repo:
        ...
        with repo.transaction() as cur:
            draft_picks_list = repo.list_picks_by_year(draft_year)
            draft_picks = {pick["pick_id"]: pick for pick in draft_picks_list}
            swap_rights = repo.list_swaps_by_year(draft_year)
            ...
            for pick_id, pick in draft_picks.items():
                ...
                if protected and owner_team and original_team and owner_team != original_team:
                    ...
                    repo.update_pick_owner(pick_id, original_team, cursor=cur)
                    compensation_asset_id = f"FIXED_COMP_{pick_id}"
                    repo.upsert_fixed_asset(
                        {
                ...
                repo.clear_pick_protection(pick_id, cursor=cur)

            for swap in swap_rights:
                ...
                if not exercisable:
                    repo.deactivate_swap(swap_id, cursor=cur)
                    events.append(
                        {
                ...
                repo.update_pick_owner(better_pick["pick_id"], better_pick["owner_team"], cursor=cur)
                repo.update_pick_owner(worse_pick["pick_id"], worse_pick["owner_team"], cursor=cur)
                repo.deactivate_swap(swap_id, cursor=cur)
