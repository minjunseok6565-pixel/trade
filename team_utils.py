...
def _init_players_and_teams_if_needed() -> None:
    """Initialize player/team caches.

    Step 6 invariant:
    - players is keyed by **player_id (string)**.
    ...
    players_state = get_players_snapshot()
    if isinstance(players_state, dict) and players_state:
        try:
            with _repo_ctx() as repo:
                for pid, pdata in list(players_state.items()):
